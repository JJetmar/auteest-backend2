...
const doTestRun = async (test, entitySchema, variation) => {
  ...
  const jsonInput = entitySchemaToJsonSchema(entitySchema, variation);
  ...
  if (test.type === 'soap') {
    ...
    result.request =  jsf.generate(jsonInput);
    options.json = true;
  }
  ...
  const waitForResponse = new Promise((resolve) => {
    ...
    request(options, (err, res, body) => {
      ...
      const hasValidInput = !variation.includes('N');
      const hasValidOutput = (result.status >= 200 && result.status < 300);
      result.valid = (hasValidInput && hasValidOutput) || (!hasValidInput && !hasValidOutput);
      resolve(result);
    });
  });
...
exports.runTest = async (ctx, next) => {
  ...
  for (let i = 0; i < variationList.length; i++) {
    ...
    // deep-copy
    const entitySchemaDeep = JSON.parse(JSON.stringify(entitySchema));
    testResult.results.push(await doTestRun(test, entitySchemaDeep, currentVariation));
  }
